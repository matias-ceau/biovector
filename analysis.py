...
# By exercise type
exercise_stats = defaultdict(lambda: {"sets": 0, "load": 0, "phi": 0, "max_weight": 0})
for s in sets:
    ex = s["exercise"]
    exercise_stats[ex]["sets"] += 1
    exercise_stats[ex]["load"] += s["load"]
    exercise_stats[ex]["phi"] += s["phi"]
    exercise_stats[ex]["max_weight"] = max(
        exercise_stats[ex]["max_weight"], s["weight"]
    )

# Top exercises by volume
print(f"\n🔥 TOP EXERCISES BY VOLUME")
sorted_ex = sorted(exercise_stats.items(), key=lambda x: x[1]["load"], reverse=True)[
    :10
]
...
for lift in main_lifts:
    ...
    if len(sorted_months) >= 2:
        ...
        print(
            f"      Progress:       {last - first:+.1f} kg ({(last / first - 1) * 100:+.1f}%)"
        )
