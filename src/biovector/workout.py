...
class Exercise(Biovector):
    def __init__(self, ID):
        ...
        # exercise data
        self.exercise_data = self.exercises[self.exercises['ID'] == ID]
        self.name = list(self.exercise_data['Exercise'])[0]
        self.Delta = list(self.exercise_data['Delta'])[0]
        self.kappa = list(self.exercise_data['rho'])[
            0] * list(self.exercise_data['theta'])[0]
...
class Workout(Biovector):
    def __init__(self, name, program=None, t=0):
        ...
        self.name = name
        if program in 'c':
            self.program = 'à changer'
        if program == 'w':
            self.program = 'workout'
        if program == 'f':
            self.program = 'free'
    ...
    def take_input(self, user_input):
        ...
        if status:
            self.status = status  # if status='end' it quits
        # possibilities
        if self.status == 'active':
            copy = self.exercise
            if exercise:
                self.choose_exercise(exercise)
            if self.exercise != copy:
                self.weight = None
        ...
        if self.status == 'delete':
            self.delete_set(note)
            self.status == None
        if self.status == 'template':
            try:
                self.program_print(note[0], note[1])
            except:
                pass
            try:
                self.program_print(self.week, self.L)
            except:
                pass
        if self.status == 'todo':
    ...
    def redo(self, note):
        delay = 0
        try:
            to_add = self.summary.loc[len(self.summary)-note:]
        except:
            print('out of range')
    ...
    def add_set(self, weight, reps, note, delay=0):
        ...
        load = round((self.exercise.Delta*weight +
                     self.BW*self.exercise.kappa)*reps)
        if self.exercise.est1RL:
            intensity = round(bv_utils.epley(
                load/reps, reps) / self.exercise.est1RL, 2)
        else:
            intensity = 1
        ...
        if self.program == 'fileadd':
            ...
            set_dic['Timestamp'].append(
                datetime.datetime.now().timestamp()+delay)
        ...
        if note:
            set_dic['Notes'].append(note)
        else:
            set_dic['Notes'].append('')
        self.sets = pd.concat(
            (self.sets, pd.DataFrame(set_dic)), ignore_index=True)
        self.summary = pd.concat(
            (self.summary, pd.DataFrame(set_dic)), ignore_index=True)
        self.exercise.exercise_history = self.sets[self.sets['ID']
                                                   == self.exercise.ID]
        self.exercise.est1RM = max(
